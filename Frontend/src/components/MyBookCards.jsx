...
const MyBookCards = ({ bookdata }) => {
    ...
    return (
      <div className={`mt-8 border-4 border-black rounded-lg relative group bg-amber-300 ${isDeleting ? 'spin-out' : ''}`}>
        <button className="absolute top-1 right-1 text-red-600 hover:text-red-800 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-400" 
        onClick={() => handleDeleteClick()}>
                <FaTrash size={18} />
            </button>
        <div className="group-hover:hidden h-[98%]">
        ...
        <div className="absolute inset-0 bg-brown-200 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-amber-300">
